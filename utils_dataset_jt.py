...
import augmentations_jt as aug
from utils_jt import float_tuple, dirlist
...
def set_dataloaders(
    ...
    if transform_val_only is not None:
        ...
        for idx, dir_val_only in enumerate(pd_dir_val_only):
            ...
            val_only_set.set_attrs(
                batch_size=args.batch,
                shuffle=False,
                num_workers=args.workers,
            )
            val_only_loader.append(val_only_set)
    ...
    for test_domain in test_domain_idx:
        ...
        test_loader_list.append(test_set)
        ...
        for idx, pd in enumerate(run[pseudo_domain_read]):
            if "pseudo_number" not in pd:
                if transform_read in pd:
                    ...
                    pd_tf.append(Compose(transform))
                ...
                for idx2, pseudo in enumerate(pd["name"]):
                    ...
                    for idx3, number in enumerate(range(pd["pseudo_number"])):
                        ...
                        pd_tf[idx2].append(Compose(transform))
                        pd_dir[idx2].append(
                            [
                                os.path.join(
                                    dataset_path,
...
def create_transform(input_list):
    transform_list = []
    for tr_module in input_list:
        if tr_module["name"] == "resize":
            transform_list.append(
                {
                    "name": "resize",
                    "augmentation": Resize(size=tr_module["size"]),
                    "param": None,
                }
            )
            ...
            transform_list.append(
                {
                    "name": "center_crop",
                    "augmentation": CenterCrop(size=tr_module["size"]),
                    "param": None,
                }
            )
            ...
            transform_list.append(aug.Normalize(mean=tr_module["mean"], std=tr_module["std"]))
        elif tr_module["name"] == "to_tensor":
            transform_list.append(aug.ToTensor())
...
def aligned_transforms(transformations, image_list):
    ...
    for tf in transformations:
        if tf["name"] in ["resize", "center_crop"]:
            ...
            i, j, h, w = tf["augmentation"].get_params(
                img, scale=tf["param"][1], ratio=tf["param"][2]
            )
            for idx in range(len(image_list)):
                image_list[idx] = tf["augmentation"].resized_crop(
...
class ImageFolderPseudoDomains(Dataset):
    def __init__(
        self,
        roots,
        image_list,
        ...
        self.set_attrs(collate_batch=self.collate_batch)
    ...
    def __getitem__(self, index):
        ...
        for i, img_item in enumerate(images):
            current_tf = self.transforms[i]
            if isinstance(current_tf, list) and len(current_tf) == 1:
                current_tf = current_tf[0]
            if current_tf is None:
                transformed_images.append(img_item)
            else:
                transformed_images.append(current_tf(img_item))
    ...
    def collate_batch(self, batch):
        ...
        num_pd = len(images[0])
        stacked = []
        for pd_idx in range(num_pd):
            stacked.append(np.stack([img_list[pd_idx] for img_list in images], axis=0))
        targets = np.array(targets, dtype=np.int32)
        return stacked, targets, list(paths)
